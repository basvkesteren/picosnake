...
int main(int argc,char **argv)
{
    ...
    sprintf(linebuffer,"/* First element contains the sprite width, second the sprite height, both in pixels */\n");
    fwrite(linebuffer, sizeof(unsigned char), strlen(linebuffer), fptr);
    sprintf(linebuffer,"static const unsigned char sprite[%i+2] = {\n", infoheader.height * imagewidth_bytes);
	fwrite(linebuffer, sizeof(unsigned char), strlen(linebuffer), fptr);
	sprintf(linebuffer,"    %i, /* Sprite width in pixels */\n", infoheader.width);
    fwrite(linebuffer, sizeof(unsigned char), strlen(linebuffer), fptr);
    sprintf(linebuffer,"    %i, /* Sprite height in pixels */\n", infoheader.height);
    fwrite(linebuffer, sizeof(unsigned char), strlen(linebuffer), fptr);\
